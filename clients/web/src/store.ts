...
const NO_HAND_ORDER: Record<Seat, readonly string[] | null> = { a: null, b: null }
const FEED_LIMIT = 4
...
export function createGameStore(): GameStore {
  ...
  return {
    ...
    apply(action) {
      ...
      const entry = describeAction(before, action, snapshot.names, snapshot.viewerSeat)
      ...
      replace({
        ...
        feed: dealt
          ? []
          : entry
            ? [...snapshot.feed, { id: nextFeedId++, text: entry }].slice(-FEED_LIMIT)
            : snapshot.feed,
      })
    },
    selectCard(card) {
