...
test("startHand deals the reference hands for seed 42", () => {
  ...
  expect(state.stock.slice(0, 3)).toEqual(
    cards("8:diamonds", "3:diamonds", "A:spades")
  );
});
...
test("startHand is rejected while a hand is in progress", () => {
  const state = apply(initialState(42, "a"), { type: "startHand" });
  expect(advance(state, { type: "startHand" })).toEqual({
    ok: false,
    reason: "wrongPhase"
  });
});
...
test("the forced draw must come from the stock, by the forced seat", () => {
  expect(
    advance(forcedDrawState(), { type: "drawDiscard", seat: "b" })
  ).toEqual({
    ok: false,
    reason: "wrongPhase"
  });
...
test("the taken card may be discarded on a later turn", () => {
  ...
  const aDone = apply(aTook, {
    type: "discard",
    seat: "a",
    card: cards("Q:clubs")[0],
    declareGin: false
  });
  const bDrew = apply(aDone, { type: "drawStock", seat: "b" });
  const bDone = apply(bDrew, {
    type: "discard",
    seat: "b",
    card: cards("K:hearts")[0],
    declareGin: false
  });
  ...
  expect(state.discardPile[state.discardPile.length - 1]).toEqual(
    cards("4:diamonds")[0]
  );
});
    ...
    for (const seat of ["a", "b"] as Seat[]) {
      const probes: Action[] = [
        ...
        { type: "drawDiscard", seat },
        {
          type: "discard",
          seat,
          card: state.hands[seat][0] ?? cards("A:clubs")[0],
          declareGin: false
        }
      ];
      const accepted = probes
        .filter((action) => {
          ...
          return (
            result.ok ||
            (result.reason !== "wrongPhase" && result.reason !== "wrongSeat")
          );
        })
...
test("upcard actions are rejected outside the offer phases", () => {
  ...
  expect(
    advance(initialState(42, "a"), { type: "passUpcard", seat: "b" })
  ).toEqual({
    ok: false,
    reason: "wrongPhase"
  });
