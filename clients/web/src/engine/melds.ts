...
function orderMeld(meld: readonly Card[]): Card[] {
  if (meld.every((held) => held.rank === meld[0].rank)) {
    return [...meld].sort(
      (a, b) => SUITS.indexOf(a.suit) - SUITS.indexOf(b.suit)
    );
  }
  ...
  return Array.from({ length: meld.length }, (_, step) =>
    byIndex.get((start + step) % RANK_COUNT)!
  );
}
...
export function bestArrangement(hand: readonly Card[]): Arrangement {
  ...
  const meldValues = meldMasks.map((mask) =>
    values.reduce(
      (sum, value, index) => (mask & (1 << index) ? sum + value : sum),
      0
    )
  );
  let bestValue = -1;
  ...
  return {
    melds: bestMasks.map((mask) =>
      orderMeld(hand.filter((_, index) => mask & (1 << index)))
    ),
    deadwood: hand
...
export function minDeadwood(hand: readonly Card[]): number {
  return bestArrangement(hand).deadwood.reduce(
...
export function ginDiscards(hand: readonly Card[]): Card[] {
  return hand.filter(
    (_, discardIndex) =>
      minDeadwood(hand.filter((_, keptIndex) => keptIndex !== discardIndex)) ===
      0
  );
}
