...
export async function createTableScene(
  ...
  await Promise.all(
    newDeck().map(async (deckCard) => {
      faces.set(
        cardKey(deckCard),
        await Assets.load<Texture>(cardAssetUrl(deckCard))
      );
    })
  );
  ...
  const buildTargets = (): BuildResult => {
      ...
      const show = (arrangement: Arrangement, y: number) => {
        ...
        for (const placed of groupedXs(
          ...
          cards.push({
            ...
            w: cardW,
            h: cardH,
            tint: deadwoodKeys.has(cardKey(placed.held))
              ? DEADWOOD_TINT
              : NO_TINT,
            baseZ: Z_HAND,
            spawn: null,
    ...
    groupedXs([game.hands[otherSeat(perspective)]], width, metrics).forEach(
      (placed, index) => {
        cards.push({
          key: `opp:${index}`,
          texture: back,
    ...
    for (const placed of groupedXs(currentHandGroups(), width, metrics)) {
      ...
      const raised =
        snapshot.selectedCard && sameCard(held, snapshot.selectedCard)
          ? metrics.raise
          : 0;
      cards.push({
        key,
  ...
  const onDragMove = (event: FederatedPointerEvent) => {
    ...
    const hand = orderHand(
      last.snapshot.game.hands[last.perspective],
      drag.order
    );
  ...
  const tick = (ticker: Ticker) => {
    ...
    for (const [key, entry] of managed) {
      ...
      if (dx * dx + dy * dy < SNAP_PX * SNAP_PX) {
        ...
        sprite.y += dy * k;
        sprite.zIndex =
          entry.baseZ + (dx * dx + dy * dy > LIFT_PX * LIFT_PX ? Z_FLIGHT : 0);
      }
    }
    ...
    update(snapshot, perspective, ginKeys) {
      ...
      if (game && game.phase !== "handOver") {
        if (snapshot.autoGroup) {
          ...
          handGroups = [
            orderHand(game.hands[perspective], snapshot.handOrder[perspective])
          ];
        }
      }
      ...
      if (won && !celebrated) {
        celebrated = true;
        if (!reducedMotion)
          fireworks.burst(app.screen.width, app.screen.height);
