...
import { cardLabel, describeAction, resultBanner, statusLine } from './status.ts'
...
test('the upcard offer explains both choices', () => {
  expect(statusLine(stateWith({ phase: 'upcardOfferNonDealer', toAct: 'a' }), NAMES, 'a')).toBe(
    'Your turn — take the upcard or pass',
  )
  expect(statusLine(stateWith({ phase: 'upcardOfferDealer', toAct: 'b' }), NAMES, 'a')).toBe(
    'Waiting for Bob to take or pass',
  )
})

test('the forced stock draw says so in both voices', () => {
  expect(statusLine(stateWith({ phase: 'forcedStockDraw', toAct: 'a' }), NAMES, 'a')).toBe(
    'You must draw from the stock',
  )
  expect(statusLine(stateWith({ phase: 'forcedStockDraw', toAct: 'b' }), NAMES, 'a')).toBe(
    'Bob must draw from the stock',
  )
})

test('a gin win reads as a win for whoever is looking', () => {
  const won = stateWith({ phase: 'handOver', toAct: null, result: { type: 'gin', winner: 'a', margin: 24 } })
...
test('a dead hand explains why nobody won', () => {
  const dead = stateWith({ phase: 'handOver', toAct: null, result: { type: 'dead' } })
...
test('the banner celebrates only the viewer', () => {
  ...
  expect(resultBanner(result, NAMES, 'a')).toMatchObject({ won: true, title: 'Gin! You win' })
  expect(resultBanner(result, NAMES, 'b')).toMatchObject({ won: false, title: 'Alice wins' })
})

test('the banner never celebrates a dead hand', () => {
  expect(resultBanner({ type: 'dead' }, NAMES, 'a')).toMatchObject({ won: false, title: 'Dead hand' })
})
...
test('the feed names the card taken from the discard pile', () => {
  expect(describeAction(withUpcard, { type: 'takeUpcard', seat: 'b' }, NAMES, 'a')).toBe(
    'Bob took the K♠',
  )
  expect(describeAction(withUpcard, { type: 'drawDiscard', seat: 'b' }, NAMES, 'a')).toBe(
    'Bob took the K♠ from the pile',
  )
})

test('the feed reports the viewer in the second person', () => {
  expect(describeAction(withUpcard, { type: 'takeUpcard', seat: 'a' }, NAMES, 'a')).toBe(
    'You took the K♠',
  )
})
...
test('a stock draw never names a card, for either player', () => {
  ...
  for (const seat of ['a', 'b'] as const) {
    const line = describeAction(withUpcard, { type: 'drawStock', seat }, NAMES, 'a')!
...
test('a pass and a discard read plainly', () => {
  expect(describeAction(withUpcard, { type: 'passUpcard', seat: 'b' }, NAMES, 'a')).toBe('Bob passed')
  const [queen] = cards('Q:diamonds')
  expect(
    describeAction(withUpcard, { type: 'discard', seat: 'b', card: queen, declareGin: false }, NAMES, 'a'),
...
test('a declared gin is called out in the feed', () => {
  const [queen] = cards('Q:diamonds')
  expect(
    describeAction(withUpcard, { type: 'discard', seat: 'a', card: queen, declareGin: true }, NAMES, 'a'),
  ).toBe('You went gin, discarding the Q♦')
})

test('dealing a hand produces no feed entry', () => {
  expect(describeAction(withUpcard, { type: 'startHand' }, NAMES, 'a')).toBeNull()
})
