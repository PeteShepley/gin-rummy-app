...
test('create queues until open, then sends the create message', () => {
  ...
  const transport = createRelayTransport({ store, url: 'ws://test', onEvent: () => {} })
  ...
  socket.open()
  expect(socket.sent).toEqual([{ kind: 'create', name: 'Ada', rnd: expect.any(Number) }])
...
test('a start contract boots the store at the creator seat and reports started', () => {
  ...
  const transport = createRelayTransport({ store, url: 'ws://test', onEvent: (e) => events.push(e) })
  const socket = MockWebSocket.last!
  socket.open()
  transport.create('Ada')
...
test('stamped actions apply in order; submit never applies locally', () => {
  ...
  expect(store.getSnapshot().game?.phase).toBe('awaitingStart')
  expect(socket.sent).toContainEqual({ kind: 'submit', action: { type: 'startHand' } })
...
test('resync rebuilds the store from the contract plus the full log', () => {
  ...
  reference.start({ seed: 42, dealer: 'a', viewerSeat: 'b', names: contract.names })
