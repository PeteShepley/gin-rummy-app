...
export type HandResult =
  { type: "gin"; winner: Seat; margin: number } | { type: "dead" };
...
export type AdvanceResult =
  { ok: true; state: EngineState } | { ok: false; reason: RejectReason };
...
function takeUpcard(state: EngineState, seat: Seat): AdvanceResult {
  const rejection = actionRejection(state, seat, [
    "upcardOfferNonDealer",
    "upcardOfferDealer"
  ]);
  if (rejection) return { ok: false, reason: rejection };
  if (state.discardPile.length === 0)
    return { ok: false, reason: "discardEmpty" };
...
function passUpcard(state: EngineState, seat: Seat): AdvanceResult {
  ...
  if (state.phase === "upcardOfferNonDealer") {
    return {
      ok: true,
      state: { ...state, phase: "upcardOfferDealer", toAct: state.dealer }
    };
  }
  return {
    ok: true,
    state: {
      ...state,
      phase: "forcedStockDraw",
      toAct: otherSeat(state.dealer)
    }
  };
}
