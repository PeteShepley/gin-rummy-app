...
import type { Contract, ErrorReason, Stamped, WireMessage } from './protocol.ts'
...
export function loadSession(): Session | null {
  try {
    ...
    return parsed.code && parsed.token && (parsed.seat === 'a' || parsed.seat === 'b')
      ? parsed
      : null
  } catch {
    return null
...
export function createRelayTransport(options: {
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    switch (message.kind) {
      case 'created':
        ...
        break
      case 'joined':
        if (pendingCode) saveSession({ code: pendingCode, token: message.token, seat: 'b' })
        break
      case 'start':
      ...
      case 'resync':
        ...
        break
      case 'error':
        if (message.reason === 'badToken' || message.reason === 'badCode') clearSession()
  ...
  socket.addEventListener('close', () => onEvent({ type: 'connection', status: 'closed' }))
