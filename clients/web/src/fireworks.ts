...
export function createFireworks(app: Application, parent: Container, zIndex: number): Fireworks {
  ...
  const launch = (x: number, y: number) => {
    ...
    for (const particle of particles) {
      ...
      const angle = (spawned / SHELL_PARTICLES) * Math.PI * 2 + Math.random() * 0.2
  ...
  return {
    ...
    update(dtMs) {
      ...
      for (const particle of particles) {
        ...
        particle.life -= dtMs
        particle.sprite.alpha = Math.max(0, Math.min(1, particle.life / FADE_MS))
        if (particle.life <= 0) {
