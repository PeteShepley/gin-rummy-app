...
function GameView({ submit, follow, noGameText, banner }: GameViewProps) {
  ...
  const handlers = {
    ...
    onHandReorder: (keys: readonly string[]) => store.setHandOrder(seatToPlay, keys),
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {game && (
        <Nameplate
          ...
          style={{ top: metrics ? `${metrics.opponentY + metrics.cardH / 2 + 8}px` : '9rem' }}
        />
      )}
      {game?.result && (
        <WinBanner
      ...
      >
        ...
        {legal.includes('discard') && selected && !discardBlocked && (
          <button type="button" style={hudButton} onClick={() => submitDiscard(false)}>
            Discard selected
          </button>
        )}
        {selected && ginKeys.has(cardKey(selected)) && (
          <button type="button" style={hudPrimaryButton} onClick={() => submitDiscard(true)}>
            Declare gin!
          </button>
        )}
        ...
        {legal.includes('discard') && selected && discardBlocked && (
          <span style={hint}>the card you just took cannot go straight back</span>
        )}
...
function App() {
  ...
  if (mode === 'solo') {
    return <GameView submit={staticSubmit!} follow="acting" noGameText="no game" />
  }
